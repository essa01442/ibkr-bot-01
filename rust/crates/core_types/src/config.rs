...
"#;
    ...
    fn test_dashboard_security_default_localhost() {
        let default_config = AppConfig {
            ...
            execution: ExecutionConfig { cancel_timeout_ms: 5000 },
            pricing: PricingConfig { k_atr: 2.0, min_stop_pct: 0.012, min_stop_abs_usd: 0.02, anti_chase_runup_pct: 0.02, slippage_alpha: 0.5, slippage_beta: 0.3, sec_fee_rate: 0.0000278, taf_rate: 0.000166, commission_per_share: 0.005, min_net_profit_usd: 0.10 },
            regime: RegimeConfig { atr_normal_max: 0.0018, atr_caution_max: 0.0028, breadth_normal_min: 0.45, breadth_caution_min: 0.35, widening_caution_pct: 0.25, widening_riskoff_pct: 0.50 },
            session: SessionConfig { regular_open_et: "09:30".to_string(), trading_start_et: "09:45".to_string(), trading_end_et: "15:45".to_string(), regular_close_et: "16:00".to_string(), pre_after_enabled: false, pre_after_min_volume: 100000 },
            ibkr: IbkrConfig { subscription_budget: 80, subscription_warn_pct: 0.80, slow_loop_pacing_per_min: 30 },
            context: ContextConfig { volume_multiplier_2x: 2.0, volume_multiplier_3x: 3.0, sector_momentum_min_pct: 2.0, churn_window_minutes: 10, churn_max_move_pct: 0.01, snap_min_trade_count: 5 },
            mtf: MtfConfig { require_all: true, stale_data_threshold_ms: 3600000 },
            correlation: CorrelationConfig { threshold: 0.40 },
            watchlist: WatchlistConfig { demotion_cycles: 3, eviction_cycles: 5, min_quality_score: 45.0, min_volume: 500000 },
            dashboard: DashboardConfig::default(),
        };

        // Test 1: Default config -> binds to 127.0.0.1
        assert_eq!(default_config.dashboard.bind_address, "127.0.0.1:8080");
    ...
    pub fn validate_dashboard_security(bind_address: &str, allow_insecure_remote: bool) -> Result<(), String> {
        ...
        if !is_localhost {
            // Test 2: Non-localhost config -> Logs security warning (simulated by returning a specific string that can be tested)
            let warning = format!("SECURITY WARNING: Dashboard is configured to bind to a non-localhost address ({})", bind_address);
            if !allow_insecure_remote {
                // Test 3: ws:// on non-localhost without override -> startup fails
                return Err(format!("{} | FATAL: ws:// on non-localhost is disabled by default for security. Set dashboard.allow_insecure_remote = true to override.", warning));
            }
            return Err(warning); // Returns warning but allows startup if we handle it
        }
        Ok(())
    }

    #[test]
    fn test_dashboard_security_non_local_warns_and_fails_if_not_allowed() {
        // Test 3: Fails when insecure remote not allowed
        let res = validate_dashboard_security("0.0.0.0:8080", false);
        assert!(res.is_err());
        assert!(res.unwrap_err().contains("FATAL"));
    }

    #[test]
    fn test_dashboard_security_non_local_warns_but_succeeds_if_allowed() {
        // Test 2: Logs warning but doesn't fail fatally when allowed
        let res = validate_dashboard_security("0.0.0.0:8080", true);
        assert!(res.is_err()); // Emitting the warning
        let err_msg = res.unwrap_err();
        assert!(err_msg.contains("SECURITY WARNING"));
        assert!(!err_msg.contains("FATAL"));
    }
